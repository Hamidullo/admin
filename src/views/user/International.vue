...
export default {
  ...
  methods: {
    ...
    deleteItemConfirm () {
      this.overlay = true
      axios.delete(`http://172.16.10.5:3002/api/internationals/delete?id=${this.editedItem.id}&newId=${this.editedItem.newId}`)
        .then(response => {
    ...
    deleteCItemConfirm () {
      this.overlay = true
      axios.delete(`http://172.16.10.5:3002/api/internationals/delete?id=${this.editedCItem.id}&newId=${this.editedCItem.newId}`)
        .then(response => {
    ...
    async save () {
      ...
      if (valid) {
        if (this.editedIndex > -1) {
          ...
          axios.put("http://172.16.10.5:3002/api/internationals/update?id="+this.editedItem.id, data)
            .then(response => {
          ...
          axios.post("http://172.16.10.5:3002/api/internationals/create?userId="+this.userId, formData)
            .then(response => {
              console.log(response.data)
    ...
    async saveC () {
      ...
      if (valid) {
        if (this.editedCIndex > -1) {
          ...
          axios.put("http://172.16.10.5:3002/api/internationals/update?id="+this.editedCItem.id, data)
            .then(response => {
              console.log(response.data)
    ...
    downloadDoc(item) {
      ...
      this.downloadWithAxios("http://172.16.10.5:3002/api/internationals/download?userId="+item.userId+"&file="+item.internationalCertificate,item.internationalCertificate)
    },
  ...
  mounted() {
    axios
      .get(`http://172.16.10.5:3002/api/internationals/type?userId=${this.userId}&limit=10&offset=0&type=61`)
      .then(response => {
    ...
    axios
      .get(`http://172.16.10.5:3002/api/internationals/type?userId=${this.userId}&limit=10&offset=0&type=62`)
      .then(response => {
        const data  = response.data
        for (const dataKey in data) {
          if (data[dataKey].news === 1){
